...
const MOTIS_AR = ["أنت على بعد 3 دروس من إتقان هذه المهارة! استمر! 🎯", "عمل رائع! الاستمرارية هي قوتك الخارقة. 🚀", "كل درس تكمله يقربك من وظيفة أحلامك! 💪", "أنت في أفضل 10% من الطلاب! ⭐"];
const MOTIS_EN = ["You're 3 lessons away from mastering this skill! Keep going! 🎯", "Great work! Consistency is your superpower. 🚀", "Every lesson brings you closer to your dream job! 💪", "You're in the top 10% of learners! ⭐"];
...
export default function StudentDashboard() {
  ...
  const statCards = ar
    ? [
        { l: "كورساتي", v: enrolled.length, i: "📚", c: C.red },
        { l: "التقدم", v: `${totalProg}%`, i: "📊", c: C.orange },
        { l: "شهادات", v: enrolled.filter(c => (getED(c.id)?.progress || 0) === 100).length, i: "🏆", c: C.purple },
      ]
    : [
        { l: "My courses", v: enrolled.length, i: "📚", c: C.red },
        { l: "Progress", v: `${totalProg}%`, i: "📊", c: C.orange },
        { l: "Certificates", v: enrolled.filter(c => (getED(c.id)?.progress || 0) === 100).length, i: "🏆", c: C.purple },
      ];
  ...
  return (
    <div dir={dir} style={{ padding: "clamp(20px,4vw,32px) 5%" }}>
      <div style={{ marginBottom: 22, display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 12 }}>
        <div>
          <div style={{ color: C.muted, fontSize: 13, marginBottom: 3 }}>{ar ? "مرحباً بعودتك 👋" : "Welcome back 👋"}</div>
            ...
            return (
              ...
              >
                <div style={{ height: 110, background: `linear-gradient(135deg,${c.color},#321d3d)`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 32, position: "relative" }}>
                  {c.icon}
                  {prog === 100 && (
                    <div style={{ position: "absolute", inset: 0, background: "rgba(16,185,129,.4)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 24 }}>🏆</div>
                  )}
